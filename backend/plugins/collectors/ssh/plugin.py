...
class SSHCollector(BaseCollector):
    """
    Reference SSH Collector plugin.
    """
    
    @property
    def collector_id(self) -> str:
        return "ssh_cli_collector"
        
    def supported_resource_types(self) -> List[str]:
        return ["Device", "Server", "Linux"]
        
    def authenticate(self, context: CollectorContext) -> bool:
        return "username" in context.credentials and "password" in context.credentials
    ...
    def discover(self, context: CollectorContext, target: Dict[str, Any]) -> Generator[ObservationEnvelope, None, None]:
        ...
        builder.with_fact("ip_address", target["ip_address"], trust=0.9, source_hint="ssh_target")
        builder.with_fact("sys_description", "Ubuntu Linux 24.04 LTS", trust=1.0)
