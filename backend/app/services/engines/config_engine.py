...
class ConfigurationEngine(BaseEngine):
    engine_name = "ConfigurationEngine"
    engine_version = "1.1"
    ...
    def evaluate(self, request: EvaluationRequest) -> EvaluationResult:
        ...
        # Determine facts/attributes to use
        attributes = {}
        ...
        for rule in rules:
            ...
            # If attribute is missing, handle appropriately
            if actual_value is None and operator != "exists":
                rule_traces.append({
                    ...
                    "actual": None,
                    "result": "UNKNOWN",
                    "message": f"Attribute '{attribute}' not found in resource state.",
                    "evidence": []
                })
                status = "UNKNOWN"
                break
            ...
            if passed:
                passed_count += 1
                message = f"Rule passed: {attribute} {operator} {expected_value}"
            else:
                failed_count += 1
                message = f"Rule failed: Expected {attribute} {operator} {expected_value}, but was {actual_value}"
                status = "FAIL"
            ...
            if not passed:
                break # Short-circuit on first failure for sprint 2
                
        duration = int((time.time() - start_time) * 1000)
        
        trace = self._build_trace(status, resource, duration, rule_traces, rules, request.context.timestamp)
        
        # We mock facts_used for sprint 2
        facts_used = [] 
        
        return EvaluationResult(status=status, trace=trace, evidence_facts=facts_used, metrics={"duration_ms": duration})
...
EngineRegistry.register(
    ...
    EngineMetadata(
        id="configuration",
        display_name="Configuration Engine",
        description="Evaluates declarative desired state against observed infrastructure configuration facts.",
        version="1.1",
        supported_operators=["equals", "less_than", "greater_than", "exists"],
        supported_resources=["network_device"],
        documentation="Validates hardware models, firmware versions, and standard configuration values.",
        examples=["firmware equals '18.0.0'"]
    )
)
