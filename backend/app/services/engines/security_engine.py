        ...
        for rule in rules:
            ...
            if actual_value is None and operator != "exists":
                rule_traces.append({
                    ...
                    "actual": None,
                    "result": "UNKNOWN",
                    "message": f"Security attribute '{attribute}' not found.",
                    "evidence": []
                })
                status = "UNKNOWN"
                break
            ...
            if passed:
                passed_count += 1
                message = f"Security rule passed: {attribute} {operator} {expected_value}"
            else:
                failed_count += 1
                message = f"Security vulnerability: Expected {attribute} {operator} {expected_value}, but was {actual_value}"
                status = "FAIL"
...
EngineRegistry.register(
    ...
    EngineMetadata(
        ...
        description="Evaluates infrastructure against security policies, vulnerability benchmarks, and exposure rules.",
        version="1.0",
        supported_operators=["equals", "less_than", "greater_than", "exists"],
        supported_resources=["network_device"],
        documentation="Validates open ports, CVE scores, and unauthorized services.",
        examples=["cve_score less_than 7.0"]
    )
)
